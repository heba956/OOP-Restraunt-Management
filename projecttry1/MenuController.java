...
public class MenuController {
    ...
    /**
     * Background loading of menu items using Task and ExecutorService
     */
    ...
    private void addToOrder() {
        ...
        if (customer != null && !Database.tables.isEmpty()) {
            ...
            boolean shouldStartProgression = (activeOrder == null || activeOrder.getStatus() == enums.OrderStatus.PLACED);
            if (activeOrder == null) {
            ...
            // Set PLACED status and publish over TCP socket
            final int id = activeOrder.getOrderId();
            activeOrder.setStatus(enums.OrderStatus.PLACED);
            OrderStatusSocketPublisher.sendStatusUpdate("localhost", 8080, id, "PLACED");

            // Start background progression thread: PLACED -> (4s) -> PREPARING -> (4s) -> SERVED via TCP socket
            if (shouldStartProgression) {
                final Order targetOrder = activeOrder;
                new Thread(() -> {
                    try {
                        Thread.sleep(4000);
                        if (targetOrder.getStatus() == enums.OrderStatus.PLACED) {
                            targetOrder.setStatus(enums.OrderStatus.PREPARING);
                            OrderStatusSocketPublisher.sendStatusUpdate("localhost", 8080, id, "PREPARING");
                        }

                        Thread.sleep(4000);
                        if (targetOrder.getStatus() == enums.OrderStatus.PREPARING) {
                            targetOrder.setStatus(enums.OrderStatus.SERVED);
                            OrderStatusSocketPublisher.sendStatusUpdate("localhost", 8080, id, "SERVED");
                        }
                    } catch (InterruptedException ignored) {}
                }, "OrderStatusProgressThread").start();
            }
        }
